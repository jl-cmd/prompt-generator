...
Authors XML prompt artifacts through incremental multiple-choice elicitation. Sibling of `/pmin` (single-pass) and `/pmid` (validated draft). Reuses prompt-generator's validator, scope block, templates, and Outcome digest format.
...
1. **Read project files** — silent. The skill uses the initial prompt to infer relevant project files and reads them with Glob/Read. No user interaction. If the inferred file count is 0 or 1, the skill asks one AskUserQuestion to confirm scope.
...
Seven scenarios in `evaluations/`. Each JSON follows the Anthropic Agent Skills eval schema. The skill is correct when every `expected_behavior` bullet in every file passes.
